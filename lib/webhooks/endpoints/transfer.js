...
router.post('/', async (req, res) => {
  ...
  // Custom functions receive 'call' and 'args', NOT 'function_call'
  if (payload.call && payload.args) {
    ...
    const callSid = payload.call.call_id;
    
    logger.info(`Transferring call ${callSid} from ${fromNumber} to ${transferNumber}`);
    
    try {
      // Use jambonz REST API to redirect the call
      const jambonzUrl = `${process.env.JAMBONZ_REST_API_URL}/v1/Accounts/${process.env.JAMBONZ_ACCOUNT_SID}/Calls/${callSid}/redirect`;
