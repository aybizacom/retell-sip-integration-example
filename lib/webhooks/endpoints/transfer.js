...
router.post('/', async (req, res) => {
  ...
  if (payload.call && payload.args) {
    ...
    // Use the jambonz call ID from dynamic variables
    ...
    try {
      // Use correct jambonz REST API endpoint - PUT without /redirect
      const jambonzUrl = `${process.env.JAMBONZ_REST_API_URL}/v1/Accounts/${process.env.JAMBONZ_ACCOUNT_SID}/Calls/${jambonzCallId}`;
